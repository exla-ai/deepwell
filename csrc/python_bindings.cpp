...
void *get_cuda_ptr(torch::Tensor &tensor) { return tensor.data_ptr(); }
...
public:
  ...
  void initialize(int m, int n, int k, const std::string &dtype_str,
                  bool use_microscaling = false, int block_size = 32) {
    problem.m = m;
    problem.n = n;
    problem.k = k;
    ...
    if (dtype_str == "fp32")
      dtype = PrecisionType::FP32;
    else if (dtype_str == "fp16")
      dtype = PrecisionType::FP16;
    else if (dtype_str == "bf16")
      dtype = PrecisionType::BF16;
    else if (dtype_str == "mxfp8")
      dtype = PrecisionType::MXFP8;
    else if (dtype_str == "nvfp4")
      dtype = PrecisionType::NVFP4;
    else if (dtype_str == "mxfp4")
      dtype = PrecisionType::MXFP4;
  ...
  torch::Tensor gemm(torch::Tensor a, torch::Tensor b,
    ...
    kernel->gemm(get_cuda_ptr(a), get_cuda_ptr(b), c_ptr, get_cuda_ptr(d),
                 epilogue, stream);
  ...
  void set_cluster_shape(int m, int n) { kernel->set_cluster_shape(m, n); }

  double get_expected_tflops() { return kernel->get_expected_tflops(); }
};
...
public:
  PyBlackwellGroupedGemmKernel()
      : kernel(std::make_unique<BlackwellGroupedGemmKernel>()) {}

  void initialize(const std::vector<std::tuple<int, int, int>> &problem_sizes,
                  const std::string &dtype_str, bool use_microscaling = false) {
  ...
  grouped_gemm(const std::vector<torch::Tensor> &a_list,
               const std::vector<torch::Tensor> &b_list, float alpha = 1.0f,
               float beta = 0.0f) {
...
public:
  PyMicroscaleManager() = default;

  std::tuple<torch::Tensor, torch::Tensor> quantize_mxfp8(torch::Tensor input,
                                                          int block_size = 32) {
    ...
    torch::Tensor scales = torch::empty(
        {num_blocks},
        torch::TensorOptions().dtype(torch::kFloat32).device(input.device()));

    cudaStream_t stream = c10::cuda::getCurrentCUDAStream().stream();

    MicroscaleManager::quantize_mxfp8(get_cuda_ptr(input), get_cuda_ptr(output),
                                      get_cuda_ptr(scales), input.numel(),
                                      block_size, stream);

    return std::make_tuple(output, scales);
  }

  torch::Tensor dequantize_mxfp8(torch::Tensor input, torch::Tensor scales,
                                 int block_size = 32) {
    TORCH_CHECK(input.is_cuda(), "Input must be on CUDA");
    ...
    MicroscaleManager::dequantize_mxfp8(
        get_cuda_ptr(input), get_cuda_ptr(scales), get_cuda_ptr(output),
        input.numel(), block_size, stream);
...
py::dict profile_kernel(PyBlackwellGemmKernel &kernel,
...
PYBIND11_MODULE(cutlass_kernels, m) {
  ...
  py::class_<PyBlackwellGemmKernel>(m, "BlackwellGemmKernel")
      .def(py::init<>())
      .def("initialize", &PyBlackwellGemmKernel::initialize, py::arg("m"),
           py::arg("n"), py::arg("k"), py::arg("dtype"),
           py::arg("use_microscaling") = false, py::arg("block_size") = 32,
           "Initialize kernel for given problem size")
      .def("gemm", &PyBlackwellGemmKernel::gemm, py::arg("a"), py::arg("b"),
           py::arg("c") = py::none(), py::arg("alpha") = 1.0f,
           py::arg("beta") = 0.0f, "Execute GEMM operation")
      .def("enable_tmem_residency",
           &PyBlackwellGemmKernel::enable_tmem_residency,
  ...
  py::class_<PyBlackwellGroupedGemmKernel>(m, "BlackwellGroupedGemmKernel")
      ...
      .def("initialize", &PyBlackwellGroupedGemmKernel::initialize,
           py::arg("problem_sizes"), py::arg("dtype"),
           py::arg("use_microscaling") = false, "Initialize grouped kernel")
      .def("grouped_gemm", &PyBlackwellGroupedGemmKernel::grouped_gemm,
           py::arg("a_list"), py::arg("b_list"), py::arg("alpha") = 1.0f,
           py::arg("beta") = 0.0f, "Execute grouped GEMM")
  ...
  py::class_<PyMicroscaleManager>(m, "MicroscaleManager")
      .def(py::init<>())
      .def("quantize_mxfp8", &PyMicroscaleManager::quantize_mxfp8,
           py::arg("input"), py::arg("block_size") = 32,
           "Quantize tensor to MXFP8")
      .def("dequantize_mxfp8", &PyMicroscaleManager::dequantize_mxfp8,
           py::arg("input"), py::arg("scales"), py::arg("block_size") = 32,
           "Dequantize MXFP8 tensor");

  // Profiling function
  m.def("profile_kernel", &profile_kernel, py::arg("kernel"),
        py::arg("warmup_iterations") = 10, py::arg("profile_iterations") = 100,
